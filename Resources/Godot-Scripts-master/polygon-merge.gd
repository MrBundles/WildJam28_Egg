...
func mergePolygons(polyA, polyB, polyAName, polyBName):
	...
	while true:
		...
		if (mergedLines.size() >= (polyA.size()+polyB.size())*2 ): 
			...
			print("Poly merge out of control!", "Poly walk aborted at "+str(mergedLines.size())+" lines where input polys had "+str(polyA.size()+polyB.size())+" lines. \n\nPolyA: "+polyAName+" \nPolyB: "+polyBName)
			break
	...
	var mergedPoly = PoolVector2Array()
...
func isPointInsideAnotherObstacle(point, parentPoly):
	#commented out as I will not be providing level static polys
